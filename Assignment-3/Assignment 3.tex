...
Let the probability that $X\geq4.5$ is $F(X \geq 4.5)$ for a given $\lambda$.\\
where $F(X \geq a)=\int_{x=a}^{\infty}f(x)$\\
From the probability density function,
\begin{align}
    \Rightarrow F(X \geq 4.5)&=\int_{x=4.5}^{\infty}f(x)\\
    &=\int_{x=4.5}^{\infty}\frac{1}{\lambda}e^{\brak{-\frac{x}{\lambda}}}\\
    &=e^{-\frac{4.5}{\lambda}}
\end{align}
Now the probability that the given null hypothesis$(H_{0})$ is true is ,
\begin{align}
    F(X \geq 4.5)&=e^{-\frac{4.5}{3}}\\
    &=0.2231
\end{align}
...
\begin{figure}[H]
    ...
    \caption{$\Pr{(4.5\leq X \leq 100\text{(some large number)})}$}
\end{figure}
Now the probability that the given alternative hypothesis is true is,
\begin{align}
    F(X \geq 4.5)&=e^{-\frac{4.5}{5}}\\
    &=0.4066
\end{align}
